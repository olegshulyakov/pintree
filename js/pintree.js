...
function createCard(title, url, icon) {
    const card = document.createElement('div');
      card.className = 'cursor-pointer flex items-center hover:shadow-sm transition-shadow px-4 py-6 bg-white shadow-sm ring-1 ring-gray-900/5 dark:pintree-ring-gray-800 rounded-lg hover:bg-gray-100 dark:pintree-bg-gray-900 dark:hover:pintree-bg-gray-800';
    ...
    const cardTitle = document.createElement('h2');
      cardTitle.className = 'text-base font-medium mb-1 truncate dark:text-gray-400';
...
function renderNavigation(folders, container, isFirstRender = false, path = []) {
    ...
    folders.forEach((folder, index) => {
        if (folder.type === 'folder') {
          ...
          navItem.className = 'items-center group flex justify-between gap-x-3 rounded-md p-2 text-gray-600 dark:text-gray-300 hover:text-main-500 hover:bg-gray-50 dark:hover:pintree-bg-gray-800 bg-opacity-50';
...
function renderBookmarks(data, path) {
    ...
    if (folders.length > 0) {
        ...
        folderSection.className = 'grid grid-cols-3 sm:grid-cols-6 lg:grid-cols-6 xl:grid-cols-8 2xl:grid-cols-12 gap-6';
